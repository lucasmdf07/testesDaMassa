...
// levels of secrecy
enum Control { PUBLIC, CONFIDENTIAL, SECRET};
...
const users[] =
{
   { "Bob",  "passwordBob", "Bobby Bobberson", SECRET },
   { "Hans", "passwordHans", "Hansel and Grettle", SECRET },
   { "Sam",  "passwordSam", "Samual Stevenson", CONFIDENTIAL },
   { "Sue",  "passwordSue", "Susan Bakersfield", CONFIDENTIAL },
   { "Sly",  "passwordSly", "Sylvester Stallone", CONFIDENTIAL }
};
...
public:
   ...
   void   displayScores(int user); // display scores on screen
   void   editScores(int user, int iSelected); // interactively edit score
 ...
 *******************************************/
void StudentGrade::editScores(int user, int iSelected)
{
 ...
 * Display scores
 ***********************************************/
void StudentGrade::displayScores(int user)
{
   if (scores.size() == 0)
      return;
 ...
 ***********************************************/
void Interface::interact(int user)
{
   int iSelected;
   while (-1 != (iSelected = promptForStudent()))
   {
      // edit grades as necessary
      students[iSelected].editScores(user, iSelected);

   if(user > -1 &&
      (users[user].control == SECRET ||
       users[user].fullName == students[iSelected].getName()))
   {
      // show the results
      students[iSelected].displayScores(user);
   }
      // visual separater
      cout << "---------------------------------------------------\n";
   }
 ...
 * Display stuff
 *************************************************/
void Interface::display(int user)
{
   for (int i = 0; i < students.size(); i++)
      students[i].displayScores(user);
}
